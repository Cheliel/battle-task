...
const Todolist = () => {
    ...
    return (
        <LinearGradient start={{ x: -0.2, y: 0 }} end={{ x: 1, y: 1 }} colors={['rgba(241, 65, 74, 0.3)', 'rgba(125, 88, 137, 0.3)', 'rgba(6, 160, 129, 0.3)', 'rgba(220, 223, 229, 0)']} style={styles.homecontainer}>
            ...
            <Text style={{fontWeight: 'bold'}}>Vos todolist de combat</Text>
            ...
            {homeList.map(item=>(
                    ...
                    tuduId= {item.idTodolist}
                    tuduName={item.nameToDoList}
                    completeStatus = {item.nameCollection}
                    refreshAction={queryAllUsersTudus}
                    action="note"
                    isNote={false}
                    facile={item.facile}
                    moyen={item.moyen}
                    dificile={item.dificile}
                    tresdificile={item.tresDificile}
                    isCollection={false}
                    />
                ))}
            </ScrollView>
            ...
            <View style={{alignItems: 'center', justifyContent: 'center'}}>
                ...
                <TouchableOpacity onPress={()=>dispatch(openScreen({screen: 'addtodolist'}))}>
                <View style={{height: 70, width: 70, borderRadius: 50, backgroundColor: '#06a0b1',alignItems: 'center', justifyContent: 'center'}}>
                <Image
