...
import { getUserTudus, postToDoList } from '../actions/AppActions'
...
const AddToDoList = () => {
    ...
    return (
        <LinearGradient start={{ x: -0.2, y: 0 }} end={{ x: 1, y: 1 }} colors={['rgba(241, 65, 74, 0.3)', 'rgba(125, 88, 137, 0.3)', 'rgba(6, 160, 129, 0.3)', 'rgba(220, 223, 229, 0)']} style={styles.homecontainer}>
            ...
            <Text style={{fontWeight: 'bold'}}>Créer une todolist </Text>
            ...
            <TextInput onChangeText={(val) => setTitre(val)} mainColor="#ffff" buttonText="Titre" placeholder='Nom de la todolist'/>
             <SelectList 
            ...
            <View style={{alignItems: 'center', justifyContent: 'center'}}>
                ...
                <TouchableOpacity onPress={() => {
                    postToDoList(titre, selected);
                    dispatch(openScreen({screen: 'todolist'}))
                }}>
                <View style={{height: 70, width: 70, borderRadius: 50, backgroundColor: '#06a0b1',alignItems: 'center', justifyContent: 'center'}}>
                <Image
