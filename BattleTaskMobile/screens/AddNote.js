...
import { getToDoSelectList, postNote } from '../actions/AppActions'
...
const AddNote = () => {
    ...
    return (
        <LinearGradient start={{ x: -0.2, y: 0 }} end={{ x: 1, y: 1 }} colors={['rgba(241, 65, 74, 0.3)', 'rgba(125, 88, 137, 0.3)', 'rgba(6, 160, 129, 0.3)', 'rgba(220, 223, 229, 0)']} style={styles.homecontainer}>
            ...
            <Text style={{marginLeft: 74, marginTop: -45, fontWeight: 'bold', fontSize: 14}}>Pseudo de l'user</Text>
            </View>
            ...
            <View style={{flex: 0.12, flexDirection: 'row', columnGap:5, marginTop: 15}} >
                <TouchableOpacity onPress={()=>dispatch(openScreen({screen: 'home'}))}>
            ...
            <TextInput onChangeText={(val) => setTitre(val)} mainColor="#ffff" buttonText="Titre" placeholder='Text de la note'/>
            <TextInput onChangeText={(val) => setLieu(val)} mainColor="#ffff" buttonText="Titre" placeholder='Lieu'/>
             <SelectList 
